...
from scanner import EnhancedWebSecurityScanner
...
class EnhancedScanThread(threading.Thread):
    ...
    def __init__(self, scan_id: str, url: str, timeout: int = 120, max_paths: int = -1, 
                 max_scan_time: int = None, user_id: int = None):
        """
        ...
        Args:
            ...
            max_paths: Max paths to scan, -1 for unlimited (default: -1)
            max_scan_time: Maximum total scan time in seconds (default: None = unlimited)
            user_id: User ID performing the scan
        """
        super().__init__()
        self.scan_id = scan_id
        self.url = url
        self.timeout = timeout  # Default changed to 120s
        self.max_paths = max_paths  # Default changed to -1 (unlimited)
    ...
    def run(self):
        ...
        try:
            ...
            # Initialize scanner with timeout, max_paths and max_scan_time
            scanner = EnhancedWebSecurityScanner(self.url, self.timeout, self.max_paths, self.max_scan_time)
...
def start_scan():
    ...
    timeout = int(data.get('timeout', 120))  # Default changed to 120s
    max_paths = int(data.get('max_paths', -1))  # Default changed to -1 (unlimited)
    max_scan_time = data.get('max_scan_time')  # NEW: Global scan timeout in seconds
    ...
    # Start scan thread with all parameters
    scan_thread = EnhancedScanThread(scan_id, url, timeout, max_paths, max_scan_time, session['user_id'])
    scan_thread.start()
...
@login_required
def list_all_scans():
    """List all completed scans for the user"""
    conn = sqlite3.connect('scanner_db.sqlite')
    cursor = conn.cursor()
    
    cursor.execute('''
        SELECT scan_id, target_url, start_time, vulnerabilities_count, risk_score, status
        FROM scans 
        WHERE user_id = ? AND status = 'completed'
...
def download_report(scan_id, format):
    ...
    print(f"DEBUG: Downloading report for scan_id={scan_id}, format={format}")
    
    # Check if scan results exist in memory first
    if scan_id in scan_results:
        results = scan_results[scan_id]
        print(f"DEBUG: Found scan results in memory")
        ...
        if 'vulnerabilities' in results:
            try:
                ...
                if format == 'json':
                    ...
                    report_data = {
                        ...
                        'summary': {
                            ...
                            'average_risk': results.get('average_risk_score', 0)
                        }
                    }
                    report_data = convert_datetime_to_string(report_data)
                    
                    with open(filepath, 'w', encoding='utf-8') as f:
                        json.dump(report_data, f, indent=2, ensure_ascii=False)
                    
                elif format == 'csv':
                    filename = f"vulnerabilities_{scan_id}_{timestamp}.csv"
                    filepath = os.path.join(reports_dir, filename)
                    import csv
                    with open(filepath, 'w', newline='', encoding='utf-8') as f:
                        if results['vulnerabilities']:
                            fieldnames = ['id', 'type', 'severity', 'risk_score', 'url', 'description', 'payload']
                    ...
                    filename = f"security_report_{scan_id}_{timestamp}.md"
                    filepath = os.path.join(reports_dir, filename)
                    with open(filepath, 'w', encoding='utf-8') as f:
                        ...
                        for i, vuln in enumerate(results['vulnerabilities'], 1):
                            ...
                            f.write(f"- **Risk Score:** {vuln.get('risk_score', 0):.1f}/10\n");
                ...
                print(f"DEBUG: Report generated at {filepath}")
    ...
    print(f"DEBUG: Scan results not found in memory, trying database")
    return jsonify({'error': 'Scan not found or results not available'}), 404
