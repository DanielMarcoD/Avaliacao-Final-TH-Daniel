...
class EnhancedWebSecurityScanner:
    ...
    def scan_advanced_xss(self):
        ...
        for param in params:
            for payload in XSS_PAYLOADS_ADVANCED[:15]:  # Enhanced payload testing
        ...
        for form in forms[:3]:  # Limit form testing
            ...
            for payload in XSS_PAYLOADS_ADVANCED[:10]:
                form_data = {}
                for inp in inputs:
                    name = inp.get('name', '')
    ...
    def scan_advanced_sqli(self):
        ...
        for param in params:
            for payload in SQL_PAYLOADS_ADVANCED[:20]:
                test_params = params.copy()
                test_params[param] = [payload]
                test_url = f"{parsed_url.scheme}://{parsed_url.netloc}{parsed_url.path}?{urlencode(test_params, doseq=True)}"
    ...
    def scan_with_nikto(self) -> List[Dict]:
        ...
        try:
            ...
            # Execute Nikto scan with timeout
            nikto_cmd = [
                'nikto',
                '-h', f"{host}:{port}",
                '-Format', 'txt',  # Use txt format for better compatibility
                '-nointeractive',
                '-timeout', '10'
            ]
            ...
            try:
                result = subprocess.run(nikto_cmd, capture_output=True, text=True, timeout=45)
                
                if result.returncode in [0, 1]:  # 0 = success, 1 = warnings but findings
                    ...
                    for line in lines:
                        line = line.strip()
                        # Look for findings (lines starting with + and containing relevant keywords)
                        if (line.startswith('+ ') and 
                            any(keyword in line.lower() for keyword in 
                                ['vulnerability', 'security', 'error', 'exposure', 'cgi', 'directory', 'file', 'server'])):
                            ...
                            self._add_vulnerability(
                                ...
                                'GET',
                                f"Nikto Finding: {line[2:]}",  # Remove '+ ' prefix
                                line.strip(),
                                {
                                    'tool': 'Nikto',
                    ...
                    print(f"{Fore.GREEN}✅ Nikto scan concluído - {findings_count} findings")
                    
                else:
                    print(f"{Fore.YELLOW}⚠️ Nikto retornou código {result.returncode} - usando simulação")
                    return self._simulate_nikto_integration()
                    
            except subprocess.TimeoutExpired:
                print(f"{Fore.YELLOW}⚠️ Timeout no Nikto scan - usando simulação")
                return self._simulate_nikto_integration()
                
        except Exception as e:
            print(f"{Fore.YELLOW}⚠️ Erro no Nikto scan: {e} - usando simulação")
            return self._simulate_nikto_integration()
    ...
    def scan_with_nmap(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"{Fore.CYAN}📡 Scanning {host}...")
            nm.scan(host, '80,443,8080,8443', '-sV --version-intensity 3 -T4', timeout=30)
    ...
    def scan_with_auxiliary_tools(self):
        ...
        auxiliary_results = {
            'zap': self.scan_with_zap_api(),
            'nikto': self.scan_with_nikto(), 
            'nmap': self.scan_with_nmap()
        }
